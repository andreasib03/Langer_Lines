...
class SyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        val uid = authRepository.currentUser?.uid ?: run {
            Log.w(TAG, "Utente non loggato, sync saltato")
        ...
        return try {
            ...
            analysesToSync.forEach { analysis ->
                ...
                if (!isRemoteUrl(analysis.imagePath)) {
                    Log.w(TAG, "Analisi ${analysis.id}: imagePath ancora locale, skip sync")
                    allSynced = false
                    return@forEach
                }
                ...
                if (success) {
                    ...
                    Log.d(TAG, "Sync completato per analisi ${analysis.id}")
                } else {
                    Log.e(TAG, "Sync fallito per analisi ${analysis.id}")
                    allSynced = false
                }
            }
            ...
            Log.e(TAG, "Errore durante il sync: ${e.message}", e)
            Result.retry()
        }
    }
}
